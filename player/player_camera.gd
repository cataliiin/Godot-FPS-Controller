...
var mouse_smoothness : float = 0.2 # Lower is more smoothing (recommended 0.1 - 0.2)

# Camera Input and Smoothing
...
@export_node_path("Node3D") var player_eyes_nodepath: NodePath

@onready var player_eyes = get_node(player_eyes_nodepath)

@export_node_path("Node3D") var nudge_target_nodepath: NodePath

@onready var nudge_target_node = get_node(nudge_target_nodepath)
...
func _ready():
	...
	nudge_target_y = nudge_target_node.position.y
	
	# Head bobbing params from player
...
func _physics_process(delta):
	...
	# Landing nudge effect
		if is_nudging:
			landing_nudge_timer -= delta
			if landing_nudge_timer > 0:
				if abs(nudge_target_node.position.y - nudge_target_y) > 0.001:
					nudge_target_node.position.y = lerp(nudge_target_node.position.y, nudge_target_y, delta * 10.0)
			else:
				is_nudging = false
				nudge_target_node.position.y = lerp(nudge_target_node.position.y, nudge_target_y, delta * 10.0)
...
func on_land(velocity: float) -> void:
	...
	nudge_target_y = nudge_target_node.position.y-nudge_intensity
	is_nudging = true
